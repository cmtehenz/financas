...
export const PLANNER_ROW_GRID =
  "md:grid-cols-[minmax(0,1.5fr)_5.5rem_minmax(8.75rem,auto)_8.75rem_2.5rem] md:items-center md:gap-4 md:px-1";
...
export function PlannerTransactionRow({
  ...
  return (
    <li className="group border-b border-border/70 last:border-b-0">
      <div
        className={cn(
          "grid grid-cols-[minmax(0,1fr)_auto_2.5rem] items-center gap-x-3 gap-y-1.5 py-3",
          PLANNER_ROW_GRID,
      ...
      >
        ...
        <div className="col-start-3 row-start-2 justify-self-end md:col-start-5 md:row-start-1">
          {onEdit ? (
            <button
              type="button"
              aria-label="Editar lançamento"
              title="Editar lançamento"
              className="inline-flex size-10 cursor-pointer items-center justify-center rounded-lg text-muted-foreground transition-colors hover:bg-muted hover:text-foreground md:size-9"
              onClick={onEdit}
            >
              <Pencil className="size-4" strokeWidth={1.75} aria-hidden="true" />
            </button>
          ) : (
            <span className="inline-block size-10 md:size-9" aria-hidden="true" />
          )}
        </div>
      </div>
    </li>
